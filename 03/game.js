...
require(['web3'], function(Web3) {
  const rpcURL = "https://rinkeby.infura.io/3616d3ebf2d1490f93b3ac08eeb907d7" // Your RCP URL goes here
  ...
  const address = '0x4e4d1e7fb8c9f85f57736ef52a3de9a633556f38'
  ...
  contract.methods.balanceOf(account).call((err, result) => {
    console.log("items owned: " + result)
    for (var i = 0; i < result; i++)
    {
      contract.methods.tokenOfOwnerByIndex(account, i).call((err, result) => {
        contract.methods.tokenURI(result).call((err, result2) => {
...
function create ()
{
    scoreText = this.add.text(0, 0, 'food: 0', { fontSize: '18px', fill: '#000' });
    accountText = this.add.text(0, 20, 'account: ', { fontSize: '18px', fill: '#000' });
    itemText = this.add.text(0, 40, 'items owned metadata: ', { fontSize: '18px', fill: '#000' });
